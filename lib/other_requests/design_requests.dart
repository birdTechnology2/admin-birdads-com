// lib/other_requests/design_requests.dart
...
class DesignRequestsPage extends StatefulWidget {
  @override
  _DesignRequestsPageState createState() => _DesignRequestsPageState();
}

class _DesignRequestsPageState extends State<DesignRequestsPage> {
  final CollectionReference requests = FirebaseFirestore.instance.collection('design_requests');
  String searchQuery = "";
  ...
  Future<void> _checkUserPermissions() async {
    ...
    if (savedUsername != null) {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        setState(() {
          canEditStatus = (doc.data() as Map<String, dynamic>)['permissions_design_requests_edit'] ?? false;
        });
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('طلبات التصميم - مرحبًا بك يا $userName'),
      ),
      body: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Row(
                                          ...
                                          children: [
                                            Text('رقم الطلب ID: ', style: TextStyle(fontSize: 16, color: textColor)),
                                            IconButton(
                                              icon: Icon(Icons.copy, color: textColor),
                                              onPressed: () => copyToClipboard(requestData['requestId'].toString()),
                                            ),
                                            Flexible(
                                              child: Text(requestData['requestId'].toString(), style: TextStyle(fontSize: 16, color: textColor)),
                                            ),
                                          ],
                                        ),
                                        Row(
                                          ...
                                          children: [
                                            ...
                                            IconButton(
                                              icon: Icon(Icons.copy, color: textColor),
                                              onPressed: () => copyToClipboard(requestData['firstName'].toString()),
                                            ),
                                            Flexible(
                                              child: Text(requestData['firstName'].toString(), style: TextStyle(fontSize: 16, color: textColor)),
                                            ),
                                          ],
                                        ),
                                        Row(
                                          ...
                                          children: [
                                            ...
                                            IconButton(
                                              icon: Icon(Icons.copy, color: textColor),
                                              onPressed: () => copyToClipboard(requestData['lastName'].toString()),
                                            ),
                                            Flexible(
                                              child: Text(requestData['lastName'].toString(), style: TextStyle(fontSize: 16, color: textColor)),
                                            ),
                                          ],
                                        ),
                                        Row(
                                          ...
                                          children: [
                                            ...
                                            IconButton(
                                              icon: Icon(Icons.copy, color: textColor),
                                              onPressed: () => copyToClipboard(requestData['email'].toString()),
                                            ),
                                            Flexible(
                                              child: Text(requestData['email'].toString(), style: TextStyle(fontSize: 16, color: textColor)),
                                            ),
                                          ],
                                        ),
                                        Row(
                                          mainAxisAlignment: MainAxisAlignment.start,
                                          children: [
                                            Text('الهاتف: ', style: TextStyle(fontSize: 16, color: textColor)),
                                            IconButton(
                                              icon: Icon(Icons.copy, color: textColor),
                                              onPressed: () => copyToClipboard(requestData['phone'].toString()),
                                            ),
                                            Flexible(
                                              child: Text(requestData['phone'].toString(), style: TextStyle(fontSize: 16, color: textColor)),
                                            ),
                                          ],
                                        ),
                                        Row(
                                        ...
                                        Divider(),
                                        _buildReadOnlyRow(requestData, 'facebookLink', 'الرابط', textColor),
                                        _buildReadOnlyRow(requestData, 'description', 'تفاصيل التصميم', textColor),
                                        _buildReadOnlyRow(requestData, 'status_editing', 'حالة التعديل', textColor),
                                        _buildEditableRowWithDialog(request, requestData, 'responseDetails', 'الرد بالتفاصيل', textColor),
                                        _buildReadOnlyRow(requestData, 'contentDetails_edit', 'طلب تعديل للمحتوى', textColor),
                                        _buildEditableRowWithDialog(request, requestData, 'responseDetails2', 'الرد بعد التعديل', textColor),
                                        _buildEditableRowWithDialog(request, requestData, 'notes', 'الملاحظات', textColor),
                                        _buildImagesRow(requestData, 'images', 'الصور', textColor),
                                        Row(
                                          children: [
      ...
      children: [
        ...
        Expanded(
          child: GestureDetector(
            onTap: () {
              if (fieldKey == 'facebookLink' || fieldKey == 'images') {
                _launchURL(requestData[fieldKey]?.toString() ?? '');
              }
            },
            child: Text(
              requestData[fieldKey]?.toString() ?? '',
              style: TextStyle(fontSize: 16, color: fieldKey == 'facebookLink' || fieldKey == 'images' ? Colors.blue : textColor, decoration: fieldKey == 'link' || fieldKey == 'images' ? TextDecoration.underline : null),
            ),
          ),
        ),
      ],
  ...
  Widget _buildImagesRow(Map<String, dynamic> requestData, String fieldKey, String label, Color textColor) {
    if (requestData[fieldKey] == null || requestData[fieldKey].isEmpty) {
      return Row(
        children: [
          Text('$label: ', style: TextStyle(fontSize: 16, color: textColor)),
          Flexible(
            child: Text('لا توجد صور', style: TextStyle(fontSize: 16, color: textColor)),
          ),
        ],
      );
    }

    final List<dynamic> imageUrls = requestData[fieldKey];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('$label: ', style: TextStyle(fontSize: 16, color: textColor)),
        ...
        Wrap(
          ...
          children: imageUrls.asMap().entries.map<Widget>((entry) {
            int index = entry.key + 1; // علشان نبدأ الترقيم من 1
